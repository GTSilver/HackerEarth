...
namespace Optimisation
{
    class Program
    {
        ...
        public static void bitArraySelf(string input, int length, int times)
        {
            Stopwatch sw1 = new Stopwatch();
            sw1.Start();
            for (var t = 0; t < times; t++)
            {
                //Конвертация в массив битов со сканом от начала до конца
                var array = new BitArray(input.Select(count => count == '1').ToArray());
        ...
        public static void boolArraySelf(string input, int length, int times)
        {
            Stopwatch sw1 = new Stopwatch();
            sw1.Start();
            for (var t = 0; t < times; t++)
            {
                //Конвертация в двоичный массив со сканом от начала до конца
                var array = input.Select(count => count == '1').ToArray();
        ...
        static void Main()
        {
            string input = "1010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101011";
            ...
            Console.WriteLine("Тест 7: Массив битов проверяет сам себя через сдвинутый индекс с одной стороны");
            for (var t = 0; t < tests; t++)
                bitArraySelf(input, length, times);

            Console.WriteLine();

            Console.WriteLine("Тест 8: Двумерный массив проверяет сам себя через сдвинутый индекс с одной стороны");
            for (var t = 0; t < tests; t++)
                boolArraySelf(input, length, times);
